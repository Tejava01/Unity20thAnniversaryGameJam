...
public class EnemyController : MonoBehaviour
{
    ...
    [SerializeField] private int score;         // score when defeated
    ...
    void FixedUpdate()
    {
        ...
        if (toPlayer.magnitude < range)
        {
            if (time <= 0)
            {
                ...
                GameObject attackObject = Instantiate(attackPrefab, (Vector2)transform.position + offset, Quaternion.identity);
                if (isProjectile)
                    attackObject.transform.up = toPlayer;
                time = attackCooldown;
            }
